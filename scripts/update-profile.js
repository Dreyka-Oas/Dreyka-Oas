...
//   - langues reellement utilisees (agregation des bytes de tous les repos owner, publics + prives)
//   - stack (icones deduites des langues detectees)
//   - chiffres (repos publics uniquement, etoiles, langue principale)
//   - activite recente (evenements publics)
// PROFILE_PAT (secret repo) est requis pour voir les repos prives du compte,
// GITHUB_TOKEN seul ne donne acces qu'au repo courant.

const fs = require('fs');
const path = require('path');

const USER = process.env.PROFILE_USER || 'Dreyka-Oas';
const TOKEN = process.env.PROFILE_PAT || process.env.GITHUB_TOKEN;
const README = path.join(process.cwd(), 'README.md');
const TOP_LANGS = 8;
const MAX_ACTIVITY = 6;

if (!TOKEN) { console.error('PROFILE_PAT/GITHUB_TOKEN manquant'); process.exit(1); }
...
async function fetchRepos(privacy) {
  const query = `query($login:String!,$after:String,$privacy:RepositoryPrivacy){
    user(login:$login){
  ...
  do {
    const data = await gql(query, { login: USER, after, privacy });
...
function buildStats(publicRepos, sortedAll) {
  const stars = publicRepos.reduce((a, r) => a + r.stargazerCount, 0);
  const top = sortedAll[0] ? sortedAll[0].name : '—';
  const b = [
    ['Repos_publics', String(publicRepos.length), '6C5CE7', 'github'],
    ['Etoiles', String(stars), 'F1C40F', 'github'],
    ['Langue_principale', top, '00B894', null],
    ['Langues', String(sortedAll.length), '0984E3', null],
  ];
...
async function fetchActivity() {
  ...
  if (!r.ok) return '_Aucune activité publique récente._';
...
(async () => {
  const [publicRepos, allRepos] = await Promise.all([
    fetchRepos('PUBLIC'),
    fetchRepos(null),
  ]);
  const sortedAll = aggregateLanguages(allRepos);
  let md = fs.readFileSync(README, 'utf8');
  md = inject(md, 'stack', buildStack(sortedAll));
  md = inject(md, 'stats', buildStats(publicRepos, sortedAll));
  md = inject(md, 'activity', await fetchActivity());
  fs.writeFileSync(README, md);
  console.log('OK — langues:', sortedAll.slice(0, TOP_LANGS).map(l => `${l.name} ${l.pct.toFixed(1)}%`).join(', '));
